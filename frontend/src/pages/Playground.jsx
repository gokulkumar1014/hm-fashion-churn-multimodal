...
export default function Playground() {
  ...
  const handleRandomId = async () => {
    ...
    try {
      ...
      console.error("Random ID recovery failed:", error);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-hm-light flex justify-center py-12 px-4 sm:px-6">
        ...
        <div className="w-full max-w-6xl bg-white/70 backdrop-blur-xl border border-white/40 shadow-glass rounded-sm relative min-h-[80vh]">
         
          {/* Header */}
          <div className="px-6 md:px-8 py-6 border-b border-gray-100 bg-white/80 backdrop-blur-md rounded-t-sm">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              
              <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
                <div className="shrink-0">
                  <h1 className="font-serif text-3xl md:text-3xl text-hm-black tracking-tight">Intelligence Playground</h1>
                  <p className="font-sans text-[10px] text-hm-gray uppercase tracking-widest mt-1">H&M Strategy Engine API</p>
                </div>

                <div className="flex flex-col items-start gap-1.5 border-l-0 md:border-l border-gray-200 md:pl-6 overflow-hidden max-w-full">
                  <span className="bg-slate-800 text-slate-100 px-2.5 py-1 rounded text-[9px] font-mono tracking-widest uppercase shadow-sm border border-slate-700 whitespace-nowrap">
                    // ROLE: TACTICAL DIAGNOSTIC
                  </span>
                  <span className="bg-white border border-red-100 text-[#E50012] px-2.5 py-1 rounded text-[9px] md:text-[10px] font-mono tracking-widest font-bold uppercase shadow-[0_2px_10px_rgba(229,0,18,0.1)] inline-block w-full sm:w-auto break-words md:whitespace-nowrap">
                    [TARGET: CUSTOMER RELATIONSHIP MANAGEMENT(CRM) / CUSTOMER SUCCESS]
                  </span>
                </div>
              </div>

              <div className="flex gap-2 items-center self-start lg:self-auto shrink-0 bg-gray-50/50 px-3 py-1.5 rounded-full border border-gray-100">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span>
                </span>
                <span className="font-sans text-[9px] text-hm-gray font-bold uppercase tracking-widest mt-[1px]">Model Active</span>
              </div>
          ...
          <div className="px-8 py-5 border-b border-gray-100 bg-white/30 text-[13px] md:text-sm text-gray-600 font-sans leading-relaxed">
            <p className="max-w-4xl border-l-2 border-gray-300 pl-4 py-1">
              A high-fidelity CRM interface for surgical customer retention. This tactical engine allows Sales and Service teams to transition from macro trends to micro-interventions, providing a 360° 'Style DNA' dossier for every unique identity.
            </p>
          </div>
        ...
        <div className="px-6 lg:px-8 py-6">
          <div className="flex flex-col gap-6 lg:flex-row">
            {/* Data Sidebar */}
            <div className="lg:w-1/4 space-y-5 lg:sticky lg:top-24 lg:self-start">
              <div className="bg-white/90 border border-gray-100 rounded-3xl shadow-2xl p-6 space-y-6">
                <div className="flex flex-col gap-2">
                  ...
                  <p className="text-sm text-hm-gray mt-1">The dataset historically spans from <strong>2018-09-19</strong> to <strong>2020-09-21</strong>.</p>
                </div>
                <div className="grid gap-3">
                  <a
                    href="https://gist.github.com/gokulkumar1014/a6693250fbe8cba238e1c74093844ed5"
                    target="_blank"
                    rel="noreferrer"
                    className="group block border border-gray-200 rounded-2xl bg-white p-4 shadow-sm transition hover:border-hm-red"
                  >
                    <p className="text-[11px] uppercase tracking-[0.4em] text-hm-gray font-semibold">Mapped IDs (GitHub Gist)</p>
                    <h4 className="font-serif text-xl text-hm-black mt-2">Identity Manifest</h4>
                    <p className="text-sm text-hm-gray mt-1">Access the Identity Manifest to browse a sample list of Hex IDs from our 1.3M customer universe.</p>
                    <div className="mt-4 flex items-center justify-between text-sm font-semibold text-hm-red">
                      <span>View Sample IDs</span>
                      <ArrowRight size={16} />
                    </div>
                  </a>
                  <a
                    href="https://www.kaggle.com/competitions/h-and-m-personalized-fashion-recommendations/data"
                    target="_blank"
                    rel="noreferrer"
                    className="group block border border-gray-200 rounded-2xl bg-white p-4 shadow-sm transition hover:border-hm-red"
                  >
                    <p className="text-[11px] uppercase tracking-[0.4em] text-hm-gray font-semibold">Raw Source (Kaggle)</p>
                    <h4 className="font-serif text-xl text-hm-black mt-2">Official H&M Dataset</h4>
                    <p className="text-sm text-hm-gray mt-1">Explore the official data for customers, transactions and articles. <strong>(Requires Sign-in)</strong></p>
                    <div className="mt-4 flex items-center justify-between text-sm font-semibold text-hm-red">
            ...
            <div className="flex-1 flex flex-col gap-4">
              <div className="relative flex-1 bg-gradient-to-br from-white via-white/70 to-[#f4ecff] border border-white/60 rounded-3xl shadow-[0_30px_80px_rgba(15,23,42,0.15)] overflow-hidden flex flex-col">
                ...
                <div className="relative z-10 flex items-center justify-between px-6 py-4 border-b border-white/60">
                  <div>
                    ...
                    <p className="text-[11px] text-hm-gray">Realtime CRM narrative + Gemini insights.</p>
                  </div>
                  <button
                    type="button"
              ...
              {/* Input Bar */}
              <div className="p-4 sm:p-6 bg-white/80 backdrop-blur-lg border border-gray-100 w-full relative z-10 shrink-0 rounded-3xl sm:rounded-3xl shadow-[0_-5px_20px_rgba(0,0,0,0.03)]">
                <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:flex-row sm:items-center max-w-4xl mx-auto w-full">
                  ...
                  <button
                    ...
                    className="flex shrink-0 items-center justify-center gap-2 px-6 py-3.5 sm:py-3 rounded-full bg-black text-[10px] font-mono tracking-[0.3em] uppercase text-white shadow-xl transition-transform hover:scale-105 disabled:opacity-40 animate-pulse w-full sm:w-auto"
                  >
                  ...
                  {/* Cohesive Input Pill */}
                  <div className="flex-1 flex items-center bg-hm-light border border-gray-200 rounded-full shadow-inner overflow-hidden focus-within:border-hm-black focus-within:ring-1 focus-within:ring-hm-black transition-all p-1 w-full">
                    <input 
                      type="text" 
                      value={input}
                      ...
                      className="flex-1 bg-transparent text-hm-black font-mono text-xs sm:text-sm px-5 py-3 outline-none tracking-widest sm:tracking-[0.2em] w-full"
                    />
                    <button 
                      type="submit" 
                      disabled={isLoading || !input.trim()}
                      className="h-10 w-10 sm:h-12 sm:w-12 shrink-0 flex items-center justify-center rounded-full bg-hm-red text-white hover:bg-black transition-colors disabled:opacity-50 disabled:bg-gray-300 disabled:cursor-not-allowed group mr-0.5 sm:mr-1"
                    >
                      <Send size={16} className="group-hover:scale-110 transition-transform sm:w-[18px] sm:h-[18px]" />
                    </button>
                  </div>
